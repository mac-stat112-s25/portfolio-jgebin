...
```{r}
ggplot(starbucks, aes(x = Latitude, y = Longitude)) +geom_point(alpha = .2, size = .2)
```
...
## Part c
Starbucks is most concentrated in North America, Europe, and East Asia.
...
```{r}
# What changed in the plot? - Instead of points there are contour lines to show density 
# What changed in our code?! - We are using geom_density_2d instead of geom_point
ggplot(cma_boundaries) + 
  geom_sf() + 
  geom_density_2d(
    data = starbucks_cma,
...
ggplot(elections_by_counties, aes(map_id = county_fips, fill = median_rent)) +
  ...
  theme(legend.position = "right") + 
  coord_equal() +
  scale_fill_gradient(low="#9ac0f5", high= "#0c3a7a")
```
```{r}
ggplot(elections_by_counties, aes(map_id = county_fips, fill = median_age)) +
  ...
  theme(legend.position = "right") + 
  coord_equal() +
  scale_fill_gradient(low="#ffcc5e", high= "#94270f")
```
